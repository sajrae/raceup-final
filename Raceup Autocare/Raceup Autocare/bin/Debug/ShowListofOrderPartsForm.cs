...
namespace Raceup_Autocare
{
    public partial class ShowListofOrderPartsForm : Form
    {
        ...
        private void ShowListofOrderPartsForm_Load(object sender, EventArgs e)
        {
            ...
            {
                OleDbDataAdapter da = new OleDbDataAdapter("Select Item_Code,Item_Name,Parts_Quantity,Unit_Price,Total_Price_Parts from RepairOrderParts Where CStr(RO_Number)  = '" + ROnumberLabel.Text + "' Order by Item_Code", mscon);
        ...
        private void CheckAvailability()
        {
            ...
            for (int i = 0; i < guna2DataGridView1.Rows.Count; i++)
            {
                ...
                while (customerReader.Read())
                {
                    ...
                    {
                        //OleDbConnection connection2 = new OleDbConnection(connection);
                        //connection2.Open();
                        //int quantity = Convert.ToInt32(guna2DataGridView1.Rows[i].Cells[2].Value);
                        //string sqlQuery2 = "Update Parts Set Quantity = Quantity - '" + quantity + "' Where CStr(Item_Code) ='" + guna2DataGridView1.Rows[i].Cells[0].Value + "'";
                        //OleDbCommand cmd = new OleDbCommand(sqlQuery2, connection2);
        ...
        private void DisableCheckBTN()
        {
            ...
            customerReader = dbcon.ConnectToOleDB(selectquery);
                ...
                while (customerReader.Read())
                {
                    ...
                    {
                        OleDbConnection connection3 = new OleDbConnection(connection);
                        connection3.Open();
                        string sqlQuery2 = "Update RepairOrderParts Set Check_Parts = 'Checked' Where CStr(RO_Number) ='" + ROnumberLabel.Text.ToString() + "'";
                        OleDbCommand cmd = new OleDbCommand(sqlQuery2, connection3);
                        cmd.CommandType = CommandType.Text;
                        cmd.ExecuteNonQuery();
                        connection3.Close();
                    }
